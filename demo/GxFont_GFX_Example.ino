...
GxIO_Class io(SPI, /*CS=5*/ SS, /*DC=*/25, /*RST=*/26); // arbitrary selection of 17, 16
GxEPD_Class display(io, /*RST=*/26, /*BUSY=*/27);       // arbitrary selection of (16), 4
...
double temperature = 0; //Œ¬∂»
bool finger_f = 0;      //Ω”¥•
uint16_t heartbeat = 0; //–ƒÃ¯
void showFont(const char name[], const GFXfont *f)
{
  display.setRotation(1); //∫·∆¡œ‘ æ
  ...
  char str[32] = {0};
  char tStr[16] = {0};
  dtostrf(temperature, 2, 1, tStr);
  sprintf(str, "T:%s H:%03d F:%d", tStr, (int)heartbeat, (int)finger_f);
  display.println(str);

  // display.println("T:22.1 H:003 F:1");
...
void showFont(const char name[], const uint8_t *f)
{
  ...
  display.println("Umlaut √Ñ√ñ√ú√§√©√∂√º");
  display.update();
}
#endif
